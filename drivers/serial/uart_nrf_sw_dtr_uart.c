...
LOG_MODULE_REGISTER(dtr_uart, 4);
...
struct dtr_uart_data {
	...
	/* Work items for specific events */
...
static int power_on_uart(struct dtr_uart_data *data)
{
	...
	LOG_DBG("power_on_uart: current state %d", state);
...
static int power_off_uart(struct dtr_uart_data *data)
{
	...
	LOG_DBG("power_off_uart: current state %d", state);
...
/**************************** WORKERS **********************************/
static void rx_disabled_work_fn(struct k_work *work)
{
	struct dtr_uart_data *data = CONTAINER_OF(work, struct dtr_uart_data, rx_disabled_work);

	if (data->dtr_state == 1) {
		LOG_DBG("RX disabled event in state %d, not suspending", data->dtr_state);
		return;
	}

	if (data->rx_active || data->tx_buf) {
		LOG_DBG("Pending for RX or TX abort");
		return;
	}
...
static void dtr_timeout_work_fn(struct k_work *work)
{
	...
	case DTR_TIMEOUT_DTR_ACTIVATION:
		/* TODO: handle error */
	case DTR_TIMEOUT_DTR_IDLE:
		/* Handle DTR idle timeout directly */
		break;
	default:
...
static void uart_callback(const struct device *uart, struct uart_event *evt, void *user_data)
{
	...
	case UART_TX_DONE:
		...
		if (!data->dtr_state) {
			k_work_submit(&data->rx_disabled_work);
		}
		break;
...
static int dtr_uart_init(const struct device *dev)
{
	...
	k_work_init(&data->rx_disabled_work, rx_disabled_work_fn);
	...
	err = pm_device_state_get(cfg->uart, &state);
	if (err < 0) {
		LOG_ERR("Failed to get child UART device state: %d", err);
		return err;
	}
	if (state != PM_DEVICE_STATE_ACTIVE) {
		err = pm_device_action_run(cfg->uart, PM_DEVICE_ACTION_RESUME);
		if (err < 0) {
			LOG_ERR("Failed to resume child UART device: %d", err);
			return err;
		}
	}
...
static int api_tx(const struct device *dev, const uint8_t *buf, size_t len, int32_t timeout)
{
	...
	if (data->tx_buf || data->dtr_state == 0) {
		LOG_WRN("TX busy or DTR not asserted");
		return -EBUSY;
	}
...
static int api_rx_buf_rsp(const struct device *dev, uint8_t *buf, size_t len)
{
	...
	if (!data->rx_active) {
		LOG_DBG("Enabling RX");
		data->rx_active = true;
		ret = uart_rx_enable(config->uart, buf, len, data->rx_timeout);

		if (ret == -EBUSY) {
			/* UART already has a buffer, */
			goto release;
		}
		return ret;
	}

	ret = uart_rx_buf_rsp(config->uart, buf, len);
	if (ret < 0) {
		LOG_ERR("RX buffer response error: %d", ret);
	}
	return ret;
release:
	LOG_DBG("Releasing buffer, RX not active (dtr %d, app_rx_enable %d, rx_active %d)",
		data->dtr_state, data->app_rx_enable, data->rx_active);
	struct uart_event evt = {
		.type = UART_RX_BUF_RELEASED,
		.data.rx_buf.buf = buf,
	};
