...
static const char *lwm2m_adv_path(uint16_t inst, uint16_t res) {
	...
	int ret = snprintk(path, sizeof(path), "%d/%d/%d", LWM2M_OBJECT_ADV_FIRMWARE_ID, inst,
		 res);
...
void lwm2m_adv_firmware_set_update_state(uint16_t obj_inst_id, uint8_t state)
{
	...
	//TODO: Use set_time() when a new API comes in.
	time_t now = time(NULL);
	lwm2m_engine_set_opaque(lwm2m_adv_path(obj_inst_id, FIRMWARE_LAST_STATE_CHANGE_TIME_ID), (void*) &now, sizeof(now));
}
...
void lwm2m_adv_firmware_set_update_result(uint16_t obj_inst_id, uint8_t result)
{
	...
	const char *path = lwm2m_adv_path(obj_inst_id, FIRMWARE_UPDATE_RESULT_ID);
...
static struct lwm2m_engine_obj_inst *firmware_create(uint16_t obj_inst_id)
{
	...
	INIT_OBJ_RES_DATA(FIRMWARE_LAST_STATE_CHANGE_TIME_ID, res[obj_inst_id], i, res_inst[obj_inst_id], j,
			  &last_change[obj_inst_id], sizeof(time_t));
        INIT_OBJ_RES_OPTDATA(FIRMWARE_COMPONENT_NAME_ID,  res[obj_inst_id], i, res_inst[obj_inst_id], j);
        INIT_OBJ_RES_DATA_LEN(FIRMWARE_CURRENT_VERSION_ID, res[obj_inst_id], i, res_inst[obj_inst_id], j, &component_version[obj_inst_id], sizeof(component_version[obj_inst_id]), 0);
        //INIT_OBJ_RES_OPTDATA(FIRMWARE_LINKED_INSTANCES_ID, res[obj_inst_id], i, res_inst[obj_inst_id], j);
        //INIT_OBJ_RES_OPTDATA(FIRMWARE_CONFLICTING_INSTANCES_ID, res[obj_inst_id], i, res_inst[obj_inst_id], j);
				   ...
				   lwm2m_engine_set_data_cb_t state_callback)
{
	...
	path = lwm2m_adv_path(idx, FIRMWARE_PACKAGE_ID);
	ret = lwm2m_engine_register_pre_write_callback(path, buffer_handler);
	if (ret < 0) {
		goto fail;
	}

	path = lwm2m_adv_path(idx, FIRMWARE_PACKAGE_URI_ID);
	ret = lwm2m_engine_register_post_write_callback(path, uri_handler);
	if (ret < 0) {
		goto fail;
	}
	...
	path = lwm2m_adv_path(idx, FIRMWARE_UPDATE_ID);
	ret = lwm2m_engine_register_exec_callback(path, update_callback);
	if (ret < 0) {
		goto fail;
	}

	path = lwm2m_adv_path(idx, FIRMWARE_CURRENT_VERSION_ID);

	if (strncmp("modem", component, strlen("modem")) == 0) {
		char buf[255];
...
fail:
	LOG_ERR("Failed to create Advanced firmware object id %d, ret = %d, last path %s", idx, ret, path);
