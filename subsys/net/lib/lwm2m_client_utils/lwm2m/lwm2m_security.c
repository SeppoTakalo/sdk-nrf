...
static bool have_permanently_stored_keys = false;
...
static int load_credentials_to_modem(struct lwm2m_ctx *ctx)
{
	...
	if (lte_lc_func_mode_get(&fmode)) {
		LOG_ERR("Failed to read modem functinal mode");
		return -EFAULT;
	}
...
static int set(const char *key, size_t len_rd, settings_read_cb read_cb, void *cb_arg)
{
	...
	if (lwm2m_engine_get_res_data(key, &buf, &len, &flags) != 0) {
...
static bool server_keys_exist_in_modem()
{
	int ret;
	bool exist;
...
bool lwm2m_security_needs_bootstrap()
{
